...
between nums[i] and nums[j] (i.e., nums[j] - nums[i]), such that 0 <= i < j < n and
 nums[i] < nums[j].
...
def max_diff_btw_incr_elem(nums):
   maxdiff,l = 0,0
   for i in range(1,len(nums)):
      if l < i and nums[l] > nums[i]:
         l +=1
      if l < i:
         maxdiff = max(maxdiff,nums[i]-nums[l])
   return maxdiff if maxdiff else -1

#nums = [7,1,5,4]
#nums = [9,4,3,2]
nums = [1,5,2,10]
print(max_diff_btw_incr_elem(nums))
